...
public class UserIOConsoleImp implements UserIO {
    ...
    public BigDecimal readBigDecimal(String promptMsg) {
        while (true) {
            try {
                String input = readString(promptMsg);
                return new BigDecimal(input);
            } catch (NumberFormatException e){
                System.out.println("Wrong input. " + promptMsg);
            }
        }
    }

    @Override
    public LocalDate readDateTime(String promptMsg) {
        while (true) {
            try {
                String input = readString(promptMsg);
                return LocalDate.parse(input, DateTimeFormatter.ofPattern("MMddyyyy"));
            } catch (DateTimeParseException e) {
                System.out.println("Incorrect Date Format. " + promptMsg);
            }
        }
    }
